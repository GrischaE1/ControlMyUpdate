...
if ($AllFiles) {
    foreach ($file in $AllFiles) {
        ...
        if ($CurrentSensor -and $UpdateSensors -eq $true) {
            $header = Create-UEMAPIHeader -APIUser $APIUser -APIPassword $APIPassword -APIKey $APIKey -APIVersion 2
            $url = "https://$($APIEndpoint)/API/mdm/devicesensors/$($CurrentSensor.uuid)"
            Invoke-RestMethod $url -Method 'PUT' -Headers $header -Body $body  

            Write-Output "$($file.basename) updated"
        }
        elseif (!$CurrentSensor) {
            #upload sensor data
            $header = Create-UEMAPIHeader -APIUser $APIUser -APIPassword $APIPassword -APIKey $APIKey
        ...
        else { Write-Output "$($file.basename) No action" }
    ...
    if ($AllSensors) {
        ...
        foreach ($file in $AllFiles) {
            ...
            if ($CurrentSensor -ne "" -and $CurrentSensor -ne $null) {
                ...
                try {
                    $Response = Invoke-RestMethod $url -Method 'POST' -Headers $header -Body $body -ErrorAction SilentlyContinue
                }
                catch {}             
            }
